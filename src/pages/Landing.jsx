          ...
          <div className="rounded-3xl border border-zinc-200 bg-white shadow-xl overflow-hidden">
            <div className="grid grid-cols-2 min-h-[420px]">
              <div className="p-4 sm:p-6 md:p-8 lg:p-10 min-w-0">
                {status === 'success' ? (
                  <div className="flex flex-col items-center justify-center h-full text-center py-12">
                    ...
                    <button type="button" onClick={() => setStatus('idle')} className="mt-6 px-5 py-2.5 rounded-xl text-sm font-medium text-blue-600 hover:bg-blue-50 transition-colors">Send another message</button>
                  </div>
                ) : (
                  <form onSubmit={handleSubmit} className="space-y-5">
                    <div>
                      <label htmlFor="contact-name" className="block text-sm font-medium text-zinc-700 mb-2">Name</label>
                      <input id="contact-name" type="text" name="name" required value={formData.name} onChange={handleChange} className={inputClass} placeholder="Your name" />
                    </div>
                    <div>
                      <label htmlFor="contact-email" className="block text-sm font-medium text-zinc-700 mb-2">E-mail</label>
                      <input id="contact-email" type="email" name="email" required value={formData.email} onChange={handleChange} className={inputClass} placeholder="your@email.com" />
                    </div>
                    <div>
                      <label htmlFor="contact-message" className="block text-sm font-medium text-zinc-700 mb-2">Message</label>
                      <textarea id="contact-message" name="message" required rows={4} value={formData.message} onChange={handleChange} className={`${inputClass} resize-y min-h-[100px]`} placeholder="Enter your message..." />
                    </div>
                    <label className="flex items-start gap-3 cursor-pointer">
                      <input type="checkbox" name="agree" checked={formData.agree} onChange={handleChange} required className="mt-1 w-4 h-4 rounded border-2 border-zinc-300 bg-white text-blue-500 focus:ring-2 focus:ring-blue-500/30 accent-blue-500" />
                      <span className="text-sm text-zinc-600">I agree to the processing of the personal data provided</span>
                    </label>
                    <button type="submit" disabled={status === 'submitting'} className="w-full py-4 rounded-2xl font-bold text-white uppercase tracking-wider bg-gradient-to-b from-blue-400 to-blue-600 shadow-lg shadow-blue-500/30 hover:shadow-blue-500/40 hover:from-blue-300 hover:to-blue-500 transition-all disabled:opacity-70 disabled:cursor-not-allowed">
              ...
              <div className="p-4 sm:p-6 md:p-8 lg:p-10 bg-zinc-50 border-l border-zinc-200 flex flex-col justify-center min-w-0">
                <h3 className="text-2xl md:text-3xl font-bold text-blue-600 uppercase tracking-wider">Contact us</h3>
                <p className="mt-4 text-zinc-600 leading-relaxed">Customer satisfaction is our top priority! Our support service is available to assist you with any questions about our services: software development, consulting, project inquiries, and more.</p>
                <p className="mt-3 text-zinc-500 text-sm">You can contact us any way you prefer:</p>
                <div className="mt-6 flex gap-3">
                  {CONTACT_ICONS.map((item) => (
                    <a key={item.label} href={item.href} aria-label={item.label} className="w-12 h-12 rounded-full border-2 border-blue-200 bg-white flex items-center justify-center text-blue-600 hover:border-blue-500 hover:bg-blue-50 transition-all">
                      {item.icon === 'mail' ? (
                ...
                <div className="mt-8 opacity-40">
                  <svg viewBox="0 0 200 80" className="w-full h-24 text-blue-300" fill="none" stroke="currentColor" strokeWidth="0.5">
                    <circle cx="30" cy="40" r="2" fill="currentColor" /><circle cx="100" cy="25" r="2" fill="currentColor" /><circle cx="170" cy="40" r="2" fill="currentColor" /><circle cx="100" cy="55" r="2" fill="currentColor" />
                    <line x1="30" y1="40" x2="100" y2="25" /><line x1="100" y1="25" x2="170" y2="40" /><line x1="170" y1="40" x2="100" y2="55" /><line x1="100" y1="55" x2="30" y2="40" /><line x1="100" y1="25" x2="100" y2="55" />
                  </svg>
                </div>
              </div>
            </div>
